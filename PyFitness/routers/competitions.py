...
async def add_competition_post(request: Request):
    ...
    while f"race_{i}" in formData:
        ...
        type_ = formData.get(f"type_{i}", "").strip()
        distance = formData.get(f"distance_{i}", "").strip()
        date = formData.get(f"date_{i}", "").strip()
        description = formData.get(f"description_{i}", "").strip()

        errors = validate_competition(race, type_, date, distance, description)
        if errors:
            return RedirectResponse(
                url=f"/competitions?error={errors[0].replace(' ', '+')}",
                status_code=303
            )
